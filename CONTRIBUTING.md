...
- Set up with `python tools/fetch_libs.py`, then `gradlew build`. `gradlew runClient` gives you a
  client with every supported addon; `gradlew runGameTestServer` runs the end-to-end conversion
...
- Addon support goes through the public API in `api/` (`PatternTranslator`, `PatternEncoder`,
  `KeyBridge`) and must be guarded by a config toggle and a mod-loaded check in `compat/AddonCompat`.
